...
class User {
  ...
  _sendFloorStatus(floorId, status) {
    console.log('[bfcp-server] Sending FloorStatus to user ' + this.id
     + ' with status ' + status);
    if(status) {
      this._sendMessage(this.bfcpUser.floorStatusMessage(floorId, RequestStatusValue.Granted));
    } else {
      this._sendMessage(this.bfcpUser.floorStatusMessage(floorId, RequestStatusValue.Released));
    }
  }
  ...
  _sendFloorRequestStatus(status) {
    console.log('[bfcp-server] Sending FloorRequestStatus to user ' + this.id
     + ' with status ' + status);
    if(status) {
      this._sendMessage(this.bfcpUser.floorRequestStatusMessage(this.floorRequestQueue.pop(0), this.wantedFloorId, RequestStatusValue.Granted));
    } else {
      this._sendMessage(this.bfcpUser.floorRequestStatusMessage(this.floorReleaseQueue.pop(0), this.wantedFloorId, RequestStatusValue.Released));
    }
  }
  ...
  _receiveMessage(message) {
    try {
      ...
      switch(message.commonHeader.primitive) {
        ...
        default:
          console.log('[bfcp-server] Unkwown message received.');
          break;
      }
    } catch(error) {
      console.log(error);
