...
const fs = require('fs');
const BFCPLib = require('lib-bfcp');
...
class User extends EventEmitter {
   ...
   * @param {Integer} serverPort        The BFCP endpoint port in the server
   ...
   */
  constructor(id, port, serverPort, ip, conferenceId, transportProtocol, bfcpServer) {
    super();
    this._id = id;
    this._port = port;
  ...
  /**
   * Gets the User server port. Will be used to send messages to the endpoint
   * through the UDP/TCP connection.
   * @return {Integer} The port
   */
  get serverPort() {
  ...
  _handleBfcpUdpConnectionMessages(port) {
    this.bfcpConnection.on('close', () => {
      this.bfcpServer.logger.info('[BFCP-SERVER] Bfcp UDP connection at port ' +
        this.serverPort + ' for user ' + this.id + ' closed.');
      this.emit('close');
    });
  ...
  _handleBfcpTcpConnectionMessages(tcpServer, port) {
    this.bfcpConnection.on('close', () => {
      this.bfcpServer.logger.info('[BFCP-SERVER] Bfcp TCP connection at port ' +
        this.serverPort + ' for user ' + this.id + ' closed.');
      tcpServer.close();
      this.emit('close');
    });
  ...
  async _startBfcpUdpConnection() {
    ...
    return await new Promise((resolve) => {
      udpServer.bind(this.serverPort, User.getServerIp(), () => {
  ...
  _startBfcpTcpConnection() {
    ...
    tcpServer.listen(this.serverPort, User.getServerIp());
